...
**适用校区：已在北京航空航天大学学院路校区验证登录与联网；沙河校区尚未验证，不能保证直接适用。** 不同校区的认证入口和 `ac_id` 可能不同，请以当地实际门户为准。
...
门户认为本次 challenge 已过期或失效。先停止高负载测试，排除其他自动登录客户端或网页登录的并发影响，再使用新 challenge 进行一次登录。可能与处理延迟、网络或门户会话有关。
